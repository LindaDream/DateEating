...
- (void)addSubViewWithFrame:(CGRect)frame
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        image.tag = 2600 + i;
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((kUIScreenWidth - 250 * kMyWidth) / 2, 0, 250 * kMyWidth, 30)];
        titleLabel.font = [UIFont systemFontOfSize:15];
        titleLabel.numberOfLines = 0;
        titleLabel.tag = 2700 + i;
        titleLabel.textColor = [UIColor blackColor];
        UILabel *priceLabel = [[UILabel alloc] initWithFrame:CGRectMake((kUIScreenWidth - 200 * kMyWidth) / 2, 40, 200 * kMyWidth, 30)];
